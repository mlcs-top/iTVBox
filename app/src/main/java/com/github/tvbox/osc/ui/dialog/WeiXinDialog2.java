...
public class WeiXinDialog2 extends BaseDialog {
    ...
    public WeiXinDialog2(@NonNull @NotNull Context context) {
        super(context);
        setContentView(R.layout.weixin_about);
        ivWeiXi = findViewById(R.id.iv_WeiXi);
        InitBean initBean = MMkvUtils.loadInitBean("");
        if (initBean != null && ToolUtils.getIsEmpty(initBean.msg.uiButtonadimg3)) {
            setNetworkBitmap(initBean.msg.uiButtonadimg3, context);
        }else{
